...
function setContactFormTabIndex(n) {
  n
    ? ($("#billingDistrict").is(":disabled") ||
        setTabIndex($("#select2-billingDistrict-container").parent(), -1),
      $("#billingWard").is(":disabled") ||
        setTabIndex($("#select2-billingWard-container").parent(), -1),
      setTabIndex($("#shipping-address input"), 0),
      $("#shippingProvince").is(":disabled") ||
        setTabIndex($("#select2-shippingProvince-container").parent(), 0),
      $("#shippingDistrict").is(":disabled") ||
        setTabIndex($("#select2-shippingDistrict-container").parent(), 0),
      $("#shippingWard").is(":disabled") ||
        setTabIndex($("#select2-shippingWard-container").parent(), 0))
    : ($("#billingDistrict").is(":disabled") ||
        setTabIndex($("#select2-billingDistrict-container").parent(), 0),
      $("#billingWard").is(":disabled") ||
        setTabIndex($("#select2-billingWard-container").parent(), 0),
      setTabIndex($("#shipping-address input"), -1),
      $("#shippingProvince").is(":disabled") ||
        setTabIndex($("#select2-shippingProvince-container").parent(), -1),
      $("#shippingDistrict").is(":disabled") ||
        setTabIndex($("#select2-shippingDistrict-container").parent(), -1),
      $("#shippingWard").is(":disabled") ||
        setTabIndex($("#select2-shippingWard-container").parent(), -1));
}
...
(function (n) {
  ...
  n.InputPhone = (function () {
    ...
    var r = "ZZ",
      ...
      i;
    return (
      (t.prototype._initSelect2 = function () {
        ...
        t
          ? ((n = t),
            (this._old_region = n),
            this.$phone_region.val(n).trigger("change"))
          : (this._old_region = n);
      }),
      (t.prototype._handleChangePhone = function (t) {
        ...
        i !== r &&
          ((this._old_region = i), this.$phone_region.val(i).trigger("change"));
      }),
...
$(function () {
  function r(t) {
    if (t) for (var i = 0; i < t.length; i++) Twine.bind(t[i]);
  ...
  function f(n) {
    return (
      (n.shipping.province && !n.requireDistrict && !n.requireWard) ||
      (n.shipping.province && n.shipping.district && !n.requireWard) ||
      (n.shipping.province && n.shipping.district && n.shipping.ward)
    );
  }
  function e(n) {
    return (
      (n.billing.province && !n.requireDistrict && !n.requireWard) ||
      (n.billing.province && n.billing.district && !n.requireWard) ||
      (n.billing.province && n.billing.district && n.billing.ward)
    );
  }
  ...
  var n = {
    ...
    handleCheckoutKeyPress: function () {
      this.isBoundingToSaveCheckoutAbandon &&
        this.setJobToSaveCheckoutAbandon(this.deadline - Date.now() + 200);
    },
    handleCheckoutChange: function (t) {
      ...
      this.requireShipping &&
        ["province", "district", "ward"].indexOf(u) >= 0 &&
        (u == "province" && f == "billing"
          ? ((this.billing.district = ""), (this.billing.ward = ""))
          : u == "district" && f == "billing"
          ? (this.billing.ward = "")
          : u == "province" && f == "shipping"
          ? ((this.shipping.district = ""), (this.shipping.ward = ""))
          : u == "district" && f == "shipping" && (this.shipping.ward = ""),
        i(n)
      ...
      (r.name == "email" || r.name == "billingPhone") &&
      this.shouldSaveCheckoutAbandon
        ? this.setJobToSaveCheckoutAbandon(5e3)
        : this.isBoundingToSaveCheckoutAbandon &&
          this.setJobToSaveCheckoutAbandon(
            Math.max(this.deadline - Date.now(), 3e3)
          );
    ...
    setJobToSaveCheckoutAbandon: function (n) {
      ...
      this.saveAbandonJobId = setTimeout(
        this.submitCheckoutAbandon.bind(this),
        n
      );
    },
    submitCheckoutAbandon: function () {
      if (
        ((this.isBoundingToSaveCheckoutAbandon = !1),
        ...
        $.post(
          n.attr("action"),
          n.find(":not(input[name=reductionCode])").serialize()
        );
      }
    },
    ...
    handleReductionCodeKeyPress: function (n) {
      n.keyCode == 13 &&
        (n.preventDefault(), n.target.value) &&
        this.applyReductionCode();
    },
    ...
    applyReductionCode: function () {
      var r = document.forms.checkoutForm,
        ...
        t,
        i;
      this.differenceAddress
        ? n.push("shippingProvince", "shippingDistrict", "shippingWard")
        : n.push("billingProvince", "billingDistrict", "billingWard");
      ...
      i = {
        refreshOnSuccess:
          "refreshDiscount refreshOrderTotalPrice refreshShipping refreshError",
    ...
    refreshShippingMethod: function (n) {
      ...
      this.differenceAddress
        ? t.push("shippingProvince", "shippingDistrict", "shippingWard")
        : t.push("billingProvince", "billingDistrict", "billingWard");
      r = {
        refreshOnSuccess:
          "refreshOrderTotalPrice refreshDiscount refreshShipping refreshError",
    ...
    handleToggleDifferenceAddress: function (r) {
      if (
        (!t(this.billing.province, this.shipping.province) ||
          !t(this.billing.district, this.shipping.district) ||
          !t(this.billing.ward, this.shipping.ward)) &&
        (this.shipping.province || this.shipping.district || this.shipping.ward)
      ) {
        if (!i(n)) {
          this.isAddressSelecting = !0;
          return;
    ...
    handleCustomerAddressChange: function (n, t) {
      ...
      t.billingProvince &&
        ((t.billingProvince.value = this.billing.province),
        t.billingProvince.setAttribute("value", this.billing.province));
      t.billingDistrict &&
        ((t.billingDistrict.value = this.billing.district),
        t.billingDistrict.setAttribute("value", this.billing.district));
      t.billingWard &&
        ((t.billingWard.value = this.billing.ward),
      ...
      Address.refresh(function () {
        r.requireShipping &&
          !r.differenceAddress &&
          (i(r)
            ? ((r.isLoadingShippingMethod = !0),
              (r.isAddressSelecting = !1),
              r.refreshShippingMethod(t))
            : (r.isAddressSelecting = !0));
      });
    },
    markShippingMethodSelectFirstIfNeed: function () {
      ...
      n.length &&
        n.indexOf(this.shippingMethod) === -1 &&
        ((this.shippingMethod = n[0]), Twine.refresh());
    },
    ...
    startPollingCheckoutStatus: function (n, t, i, r) {
      function f() {
        fetch(n, { headers: { "X-Requested-With": "FetchApiRequest" } })
          .then(function (n) {
            var t = n.headers.get("X-Next-Redirect");
            if (t) return Page.visit(t, { reload: !0 }), !0;
            u.timeoutIds.push(
              setTimeout(function () {
                f();
              }, 1e3)
    ...
    sendCheckoutForm: function (n, t) {
      for (
        ...
        var r = s[f],
          v =
            !r.disabled &&
            ((t.onlyKeys && t.onlyKeys.indexOf(r.name) >= 0) ||
              (t.exceptKeys && t.exceptKeys.indexOf(r.name) < 0) ||
              (!t.onlyKeys && !t.exceptKeys)),
    ...
    onSuccess: function (n, t, i) {
      ...
      if (r) {
        Page.visit(r, { reload: !0 });
        return;
      }
      t
        ? Page.refresh({ response: n, onlyKeys: t })
        : i
        ? Page.refresh({ response: n, exceptKeys: i })
        : Page.refresh({ response: n });
    },
  ...
  document.addEventListener("page:before-partial-replace", function (n) {
    var i = n.data,
      t;
    if (i) for (t = 0; t < i.length; t++) Twine.unbind(i[t]);
  });
  document.addEventListener("page:load", function (t) {
    r(t.data);
    n.markShippingMethodSelectFirstIfNeed();
    n.shouldRefreshAddress &&
      (Address.refresh(), u(), (n.shouldRefreshAddress = !1));
  });
}),
  (function (n) {
    function i(n, t) {
      for (var r = /\$\(([^\)]+)?\)/g, i; (i = r.exec(n)); )
        (n =
          t[i[1]] && t[i[1]] instanceof Array
            ? n.replace(i[0], t[i[1]].join(""))
            : n.replace(i[0], t[i[1]])),
          (r.lastIndex = 0);
      return n;
    }
    ...
    var r = {
      ...
      loadBank: function (n) {
        ...
        return fetch(r)
          ...
          .then(function (r) {
            ...
            if ($(".bank-list"))
              if (
                ((o = $("#bank-list__template").text()), (e = []), n === "qr")
              ) {
                for (u = 0; u < r.banks.length; u++)
                  (f = r.banks[u]), e.push(i(o, f));
                t.bankQrList = r.banks;
                $(".bank-list").html(e.join(""));
              } else if (n === "inst") {
                for (
                  s = ["<option value hidden>Chọn ngân hàng</option>"], u = 0;
                  u < r.banks.length;
                  u++
                )
                  (f = r.banks[u]),
      ...
      loadQrCode: function () {
        ...
        n.isLoadingQr) ||
          ...
          t.sendReductionCodeForm
            ? t.sendReductionCodeForm(i, r)
            : t.sendCheckoutForm && t.sendCheckoutForm(i, r),
          Twine.refresh());
      },
      changeBankCodeInst: function (n) {
        var t = this;
        n != t.payon_bank &&
          ((t.bankInstChoose = t.bankList.find(function (t) {
            return t.code == n;
          })),
          $("#payon_bank").val(n),
          $("#payon_card").val(""),
      ...
      checkBankCardInst: function (n) {
        return this.bankInstChoose &&
          this.bankInstChoose.installment_card_type &&
          this.bankInstChoose.installment_card_type.find(function (t) {
            return t === n;
          })
          ? !0
          : !1;
      ...
      renderMobileInst: function () {
        var n = this,
          ...
          o;
        for (
          n.payon_cycle = n.listFee[0].month,
        ...
        o = {
          ...
          amount_month: t(
            (n.listFee[0].amount_payment + r) / n.listFee[0].month
          ),
      ...
      renderDesktopInst: function () {
        for (
          ...
          (n = e.listFee[f]),
            ...
            r.amount_month.push(
              '<td><span class="red">' +
                t((n.amount_payment + u) / n.month) +
                "</span></td>"
            ),
            r.amount_total.push(
              "<td><span>" + t(n.amount_payment + u) + "</span></td>"
            ),
            r.amount_deviant.push(
              "<td><span>" +
                t(u) +
                '</span><button type="button" class="btn btn-default btn-choose-cycle" bind-event-click="PayOn.submitForm(this, ' +
                n.month +
                ')">Thanh toán<svg xmlns="http://www.w3.org/2000/svg"><use href="#spinner"></use></svg></button></td>'
            );
        Twine.unbind($(".payon-inst__fee")[0]);
        o.html(i(s, r));
        Twine.bind($(".payon-inst__fee")[0]);
      ...
      submitForm: function (n, t) {
        ...
        i.sendReductionCodeForm
          ? i.sendReductionCodeForm(f, r)
          : i.sendCheckoutForm && i.sendCheckoutForm(f, r);
        Twine.refresh();
      },
    };
    n.PayOn = r;
